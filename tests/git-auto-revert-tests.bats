...
@test "git auto-revert should fail when there are staged changes" {
  ...
  [ $status -ne 0 ]
  [[ "$output" == *"error: your local changes would be overwritten by auto-revert"* ]] || false
}
...
@test "git auto-revert should revert all commits prefixed with the given commit log message tag" {
  ...
  [[ "$(git diff --name-status HEAD~1..HEAD)" =~ ^D[[:blank:]]*file1.txt$ ]] || false
  [[ "$(git diff --name-status HEAD~2..HEAD~1)" =~ ^D[[:blank:]]*file3.txt$ ]] || false
  [[ "$(git diff --name-status HEAD~3..HEAD~2)" =~ ^D[[:blank:]]*file4.txt$ ]] || false
  [ "$(git show -s --format=%s)" == "Revert \"FEAT-001 add file1\"" ]
  [ "$(git show -s --format=%s HEAD~1)" == "Revert \"FEAT-001: add file3\"" ]
  [ "$(git show -s --format=%s HEAD~2)" == "Revert \"FEAT-001 add file4\"" ]
}
...
@test "git auto-revert should revert all commits prefixed with the given conventional commit message tag" {
  ...
  [[ "$(git diff --name-status HEAD~1..HEAD)" =~ ^D[[:blank:]]*file2.txt$ ]] || false
  [[ "$(git diff --name-status HEAD~2..HEAD~1)" =~ ^D[[:blank:]]*file4.txt$ ]] || false
  [ "$(git show -s --format=%s)" == "Revert \"feat(cool-stuff) add file2\"" ]
  [ "$(git show -s --format=%s HEAD~1)" == "Revert \"feat(cool-stuff) add file4\"" ]
}
...
@test "git auto-revert with --grep should revert all commits having commit log message matching the given pattern" {
  ...
  [[ "$(git diff --name-status HEAD~1..HEAD)" =~ ^A[[:blank:]]*file2.txt$ ]] || false
  [[ "$(git diff --name-status HEAD~2..HEAD~1)" =~ ^A[[:blank:]]*file5.txt$ ]] || false
  [ "$(git show -s --format=%s)" == "Revert \"BUGFIX-001 remove bad file2.txt to resolve production issue\"" ]
  [ "$(git show -s --format=%s HEAD~1)" == "Revert \"BUGFIX-002 remove file5.txt too\"" ]
}
...
@test "git auto-revert with multiple --grep's should revert all commits having commit log message matching one of the given patterns" {
  ...
  [ -f file4.txt ]
  [ -f file5.txt ]
  [ $(($(cat file1.txt | wc -l))) -eq 1 ]
  [ $(($(cat file4.txt | wc -l))) -eq 1 ]
  [ $(($(git --no-pager log --pretty=oneline | wc -l))) -eq 9 ]
  [[ "$(git diff --name-status HEAD~1..HEAD)" =~ ^M[[:blank:]]*file1.txt$ ]] || false
  [[ "$(git diff --name-status HEAD~2..HEAD~1)" =~ ^M[[:blank:]]*file4.txt$ ]] || false
  [ "$(git show -s --format=%s)" == "Revert \"BUGFIX-001: modify content of file1.txt\"" ]
  [ "$(git show -s --format=%s HEAD~1)" == "Revert \"BUGFIX-002 edit file4.txt\"" ]
}
...
@test "git auto-revert with --grep --all-match with multiple \"--grep\"s should revert all commits having commit log message matching all of the given patterns" {
  ...
  [ $(($(cat file1.txt | wc -l))) -eq 1 ]
  [ $(($(cat file4.txt | wc -l))) -eq 1 ]
  [ $(($(cat file5.txt | wc -l))) -eq 2 ]
  [ $(($(git --no-pager log --pretty=oneline | wc -l))) -eq 10 ]
  [[ "$(git diff --name-status HEAD~1..HEAD)" =~ ^M[[:blank:]]*file1.txt$ ]] || false
  [[ "$(git diff --name-status HEAD~2..HEAD~1)" =~ ^M[[:blank:]]*file4.txt$ ]] || false
  [ "$(git show -s --format=%s)" == "Revert \"BUGFIX-001: edit content of file1.txt\"" ]
  [ "$(git show -s --format=%s HEAD~1)" == "Revert \"BUGFIX-002 edit file4.txt\"" ]
}
...
@test "git auto-revert with --grep --invert-grep should revert all commits having commit log message not matching the given pattern" {
  ...
  [ $(($(cat file1.txt | wc -l))) -eq 1 ]
  [ $(($(git --no-pager log --pretty=oneline | wc -l))) -eq 9 ]
  [[ "$(git diff --name-status HEAD~1..HEAD)" =~ ^D[[:blank:]]*file3.txt$ ]] || false
  [[ "$(git diff --name-status HEAD~2..HEAD~1)" =~ ^D[[:blank:]]*file5.txt$ ]] || false
  [[ "$(git diff --name-status HEAD~3..HEAD~2)" =~ ^M[[:blank:]]*file1.txt$ ]] || false
  [ "$(git show -s --format=%s)" == "Revert \"fix: add file3\"" ]
  [ "$(git show -s --format=%s HEAD~1)" == "Revert \"chore: add file5\"" ]
  [ "$(git show -s --format=%s HEAD~2)" == "Revert \"doc: add description to file1\"" ]
}
...
@test "git auto-revert with --grep -i should revert all commits having commit log message matching the given pattern case insensitively" {
  ...
  [[ "$(git diff --name-status HEAD~1..HEAD)" =~ ^D[[:blank:]]*file2.txt$ ]] || false
  [[ "$(git diff --name-status HEAD~2..HEAD~1)" =~ ^D[[:blank:]]*file4.txt$ ]] || false
  [ "$(git show -s --format=%s)" == "Revert \"fix: create file2\"" ]
  [ "$(git show -s --format=%s HEAD~1)" == "Revert \"FIX add file4\"" ]
}
...
@test "git auto-revert with --grep and --extended-regexp should allow reverting commits using an extended regex" {
  ...
  [[ "$(git diff --name-status HEAD~1..HEAD)" =~ ^D[[:blank:]]*file3.txt$ ]] || false
  [[ "$(git diff --name-status HEAD~2..HEAD~1)" =~ ^D[[:blank:]]*file4.txt$ ]] || false
  [ "$(git show -s --format=%s)" == "Revert \"BUGFIX-001: add file3\"" ]
  [ "$(git show -s --format=%s HEAD~1)" == "Revert \"IMPROV-001 add file4\"" ]
}
...
@test "git auto-revert with -s should should revert all matching commits and create a single commit with the message matching the default revert log message format" {
  ...
  [[ "$(git diff --name-status HEAD~1..HEAD | head -1)" =~ ^D[[:blank:]]*file1.txt$ ]] || false
  [[ "$(git diff --name-status HEAD~1..HEAD | head -2 | tail -1)" =~ ^D[[:blank:]]*file3.txt$ ]] || false
  [[ "$(git diff --name-status HEAD~1..HEAD | tail -1)" =~ ^D[[:blank:]]*file4.txt$ ]] || false
  [ "$(git show -s --format=%s)" == "Revert \"FEAT-001\"" ]
}
...
@test "git auto-revert with --no-auto-skip-empty should not skip and stop auto-revert when a revert introduces an empty commit" {
  ...
  [ -f file5.txt ]
  [ $(($(git --no-pager log --pretty=oneline | wc -l))) -eq 7 ]
  [[ "$(git diff --name-status HEAD~1..HEAD)" =~ ^D[[:blank:]]*file4.txt$ ]] || false
  [[ "$(git status -b)" == *"Revert currently in progress."* || $(git show -s --format=%H REVERT_HEAD) ]] || false
}
...
@test "git auto-revert with --verbose should print verbose output" {
  ...
  [ $status -eq 0 ]
  [[ "$output" == *"The following commits are reverted:"* ]] || false
...
@test "git auto-revert with --author should only revert matching commits authored by the given author" {
  ...
  [[ "$(git diff --name-status HEAD~1..HEAD)" =~ ^D[[:blank:]]*file3.txt$ ]] || false
  [[ "$(git diff --name-status HEAD~2..HEAD~1)" =~ ^D[[:blank:]]*file4.txt$ ]] || false
  [ "$(git show -s --format=%s)" == "Revert \"FEAT-001: add file3\"" ]
  [ "$(git show -s --format=%s HEAD~1)" == "Revert \"FEAT-001 add file4\"" ]
}
